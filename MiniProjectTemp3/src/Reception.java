...
public class Reception
{
    ...
    public void cancelAppointment(Admin a){
        System.out.println("Enter the doctor type Where the patient Assigned ");
        String s=sc.next();
        LinkedList<Integer> lst=a.searchdoctor(s);
        if(lst.size()==0){
            System.out.println("Doctor with this speciality Not found");
        }
        else{
            System.out.println("Enter the patient ID :: ");
            int patient=sc.nextInt();
            for(int i=0;i<lst.size();i++){
                for(int j=0;j<a.d.get(lst.get(i)).p.size();j++){
